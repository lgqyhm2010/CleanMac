...
> आधिकारिक डाउनलोड साइन और नोटराइज़्ड होते हैं। यदि Gatekeeper सत्यापित न कर सके, तो quarantine बंद न करें; फ़ाइल हटाकर आधिकारिक Releases पेज से दोबारा डाउनलोड करें।
...
```bash
# केवल स्थानीय, गैर-वितरण योग्य प्रीव्यू
./script/build_dmg.sh --unsigned

# औपचारिक रिलीज़; Developer ID और notarization क्रेडेंशियल आवश्यक
CLEANMAC_VERSION=1.0.0 CLEANMAC_BUILD_NUMBER=1 \
  NOTARY_PROFILE=CleanMacNotary ./script/build_dmg.sh --release
```

औपचारिक रिलीज़ fail-closed है: क्रेडेंशियल, संस्करण, architecture, signing, notarization, stapling या Gatekeeper की कोई भी विफलता build रोक देती है। कॉन्फ़िगरेशन environment variables से होता है और secrets hard-code नहीं होते:

| वेरिएबल | उद्देश्य |
|----------|---------|
| `CODESIGN_IDENTITY` | साइनिंग पहचान, जैसे `Developer ID Application: Name (TEAMID)`। यदि अनसेट है तो स्वतः पता लगाया जाता है। |
| `CLEANMAC_VERSION` / `CLEANMAC_BUILD_NUMBER` | रिलीज़ संस्करण और संख्यात्मक build number। |
...
> `--unsigned` केवल local/PR validation के लिए है और औपचारिक release के रूप में प्रकाशित नहीं होता। [`docs/RELEASING.md`](docs/RELEASING.md) देखें।

PR और `main` push केवल read-only unsigned preview बनाते हैं। सभी checks पास होने के बाद केवल `v*` tag प्रकाशित कर सकता है ([workflow](.github/workflows/release-dmg.yml))।
